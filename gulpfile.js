...
const PATHS = (() => {
    ...
    ret.dirImages = `${ret.dirSources}resource/images/`;
    ret.dirImageminDist = `${ret.dirDistSources}resource/images`;
    ...
    ret.dirScripts = `${ret.dirSources}resource/js/`;
    ret.dirScriptsDist = `${ret.dirDistSources}resource/js/`;
    ret.dirStylesheets = `${ret.dirSources}resource/css/`;
    ret.dirStylesheetsDist = `${ret.dirDistSources}resource/css/`;
    ret.dirTmp = 'tmp/';
    ret.dirTests = 'test/';
    ret.dirBrowserifyDist = `${ret.dirDistSources}resource/js/`;
...
// TODO: アイコンフォント自動生成 -> 未動作の部分あり、featuresブランチで検証
/*
 gulp.task('iconfont', () => {
...
// KSS作成
gulp.task('kss', shell.task([`kss-node --source ${PATHS.dirStylesheets} --destination ${PATHS.dirKSS} --css /resource/css/${PATHS.filenameStylesheetDist}`]));
// mochaテスト
gulp.task('mocha', () => {
    return gulp.src([PATHS.globScripts])
        .pipe(istanbul({
...
// JSの圧縮
gulp.task('uglify', () => {
    return gulp.src('dist/resource/js/scripts.js')
        .pipe(uglify())
...
gulp.task('build-postcleaning', () => {
    return del([
        ...
        './dist/**/*.scss',
        './dist/**/*.sass',
        './dist/resource/js/modules',
        './dist/resource/css/modules',
        './dist/resource/css/vendor'
    ]);
});
// ビルド
gulp.task('build', () => {
    runSequence('build-precleaning', 'build-precopying', ['browserify', 'ejs', 'imagemin', 'kss', 'sass'], ['csscomb', 'uglify'], 'build-postcleaning');
});
// テストツール
